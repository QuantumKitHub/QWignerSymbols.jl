...
Base.convert(::Type{ComplexF64}, ::Type{RootOfUnity{k}}) where {k} =
    cispi(1 // (k + 2)) # this is really q, not q^2

q(::Type{SU2kIrrep{k}}) where {k} = convert(ComplexF64, RootOfUnity{k})
...
dim(s::SU2kIrrep) = Float64(q_number(twice(s.j) + 1, q(typeof(s))^2)) # q^2 needed
